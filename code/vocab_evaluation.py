...
choosen_fold = "recovery_test"
    ...
    # choosen_fold = "recovery_test"
choosen_model ="filtered_models_9"
...
def avg_feature_vector(sentece,vocab_des,remove_from_vocab):
    sentece = sentece.split()
    sentece = [w for w in sentece if w not in stopwords]
    if remove_from_vocab:
        sentece = [w for w in sentece if w not in vocab_des]
...
def embedding_doc_vectorizer(doc_data,vocab_des, remove_from_vocab=False):
    output = np.zeros(len(doc_data)*300).reshape(len(doc_data), 300)
    for index , doc in enumerate(doc_data):
        output[index] = avg_feature_vector(doc,vocab_des,remove_from_vocab)
    return output
...
def embedding_similarity(fold):
    ...
    vocab_des = set()
    for dess in descriptions_txt:
        dess = clean_up_txt(dess)
        for w in dess.split():
            vocab_des.add(w)


    des_tfidf = embedding_doc_vectorizer(descriptions_txt,vocab_des)
    web_tfidf = embedding_doc_vectorizer(web_txt,vocab_des,True)
    accuracy, rank_index_stats = tfidf_inference(des_tfidf, descriptions_class, web_tfidf, web_class)
    return accuracy, rank_index_stats
...
def each_fold_stats():
    ...
    plt.title('Accuracy in Top N (No Vocabulary overlap)'.format(0))
    plt.ylabel('Accuracy')
    plt.xlabel('Top N')
    ...
    plt.plot(list(range(1,MAX_RANK)),np.mean(tfidf_avrg,0),label='Tf-idf cosine sim',linewidth=2, color='g')
