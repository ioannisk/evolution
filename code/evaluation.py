...
def train_naive_bayes_des_local(fold):
    ...
    vec = count_vectorization(X_train_des + X_train[:len(X_train)/2])
    X_train_vec = vec.transform(X_train_des + X_train[:len(X_train)/2])
    print(X_train_vec.shape)
    Y_train = Y_train_des + X_train[:len(X_train)/2]
    print(len(Y_train))
    X_valid_vec = vec.transform(X_valid)
    a = 0.0002
    # a =0.000001
...
def decomposable_attention_eval(fold):
    ...
    ## usual stuff
    with open("/home/ioannis/models/{}/model{}/prob_predictions_filter_l2.txt".format(choosen_model,fold), "r") as file_:
