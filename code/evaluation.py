...
def each_fold_stats():
    ...
    plt.plot(RANKS , np.mean(nb_avrg,0),label='Naive Bayes',linewidth=2)
    mean_rank = np.mean(np.asarray(bar_nb_data/len(folds))*np.asarray(RANKS)/np.sum(np.asarray(bar_nb_data/len(folds))))
    plt.axvline(x= mean_rank,linestyle='--', color='blue')


    # plt.plot(tfidf_avrg/len(folds),label='Tf-idf cosine sim',linewidth=2)
    plt.plot(RANKS, np.mean(tfidf_avrg,0),label='Tf-idf cosine sim',linewidth=2)
    mean_rank = np.mean(np.asarray(bar_tf_data/len(folds))*np.asarray(RANKS)/np.sum(np.asarray(bar_tf_data/len(folds))))
    # plt.errorbar(x=RANKS,y=np.mean(tfidf_avrg,0), yerr=np.std(tfidf_avrg,0), label='Tf-idf cosine sim',linewidth=2, color='green')
    plt.axvline(x= mean_rank,linestyle='--', color='green')

    # plt.plot(att_avrg/len(folds),label='Decomposable Attention',linewidth=2)
    plt.plot(RANKS, np.mean(att_avrg,0),label='Decomposable Attention',linewidth=2)
    mean_rank = np.mean(np.asarray(bar_da_data/len(folds))*np.asarray(RANKS)/np.sum(np.asarray(bar_da_data/len(folds))))
    # plt.errorbar(x=RANKS,y=np.mean(att_avrg,0), yerr=np.std(att_avrg,0), label='Decomposable Attention',linewidth=2, color='red')
    plt.axvline(x= mean_rank,linestyle='--', color='red')

    plt.legend(loc= 4)
    plt.show()
    # print([bar_nb_data/len(folds),bar_tf_data/len(folds),bar_da_data/len(folds)])
    plt.title('Accuracy in each Rank')
    xx = np.asarray(range(MAX_RANK))
